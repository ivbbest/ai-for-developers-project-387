...
## Точка продолжения (вечер 2026-09-05) — читать первым
...
5.10. Релиз v1.0.3 выпущен (тег `cal-com-v1.0.3`). Ревью проекта разобраны
целиком: PR #31 (операционное) + #32 (правки Р4–Р7) + #33 (стратегическое,
`docs/comprehensive-review.md` с разбором Р10–Р16 и рабочей интерпретацией
В1–В3). main — `5f7cbc8`.
...
4. ~~#33~~ — смержен (`5f7cbc8`), разбор Р10–Р16 и ответы В1–В3 в самом
   документе; из него в работу: Р11/Р15 = часть 6.11, Р16 — аудит пинов
   перенесённых CI при следующем касании (вне критического пути).
...
- [ ] #34: разбор независимого аудита (проверить находки, слить/закрыть с аргументом)
...
- [ ] Пользователь: лимит расходов в кабинете провайдера (вторая половина P.3);
      ...
      вызывающих» и закрыть P.3 по факту
- [ ] Опции после курса: `/oc fix` по #12 (критерии есть), закрытие #25 по
      цели (Performance ≥0.9, TBT <200 мс) после двух зелёных замеров
...
**Инвентарь:** PR #1–#33 закрыты/слиты; открыты #34 (независимый аудит
`docs/recommendations-audit.md` — на разборе) и #35 (эта память); issues:
#6 closed, #10
closed, #12 open (критерии есть, работа не начата), #25 open (мониторинг);
