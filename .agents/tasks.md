...
(run 34019912835, success, дедup #25 комментарием). Релизный цикл замкнут на
v1.0.3; фича «отмена брони» (#12) реализована и влита — PR #40 → main
`2e47444`, все прогоны на main зелёные (hexlet-check 34020457678, E2E
34020457663, contract-sync 34020457635, Docker 34020457633, Release please
34020457651). Closeout-PR (этот): единая архивная папка, owner-guide,
переписка реестра/памяти, ru-text-правка.

**Ожидает владельца (единственные внешние шаги):**
1. Release-PR #41 `chore(main): release cal-com 1.1.0` — прогоны в
   `action_required` (одобрение только в UI), после зелёных — мерж → тег и
   Release;
2. удалить remote-ветку `docs/memory-p3-close` (слитый #39; auto-delete не
   сработал);
...
closed (#12 — после мержа #40), #25 open (мониторинг); релиз v1.0.3 в проде,
1.1.0 за #41; в main 4 opencode-воркфлоу + 4 перенесённых CI + hexlet-check;
