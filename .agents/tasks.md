...
## Точка продолжения (итог 2026-09-06) — читать первым

**Где мы.** Курс закрыт целиком: шаги 1–6, все 28 требований user-story со
следами (таблица «Сверка требований»), ночной cron отработал планово
(run 34019912835, success, дедup #25 комментарием). Релизный цикл замкнут повторно:
фича «отмена брони» (#12) — PR #40 → main `2e47444` (прогоны зелёные),
release-PR #41: approve прогонов владельцем в UI → мерж → main `77ee618`,
тег и GitHub Release `cal-com-v1.1.0`; все 5 CI на main зелёные
(hexlet-check 34024666479, Contract sync 34024666478, E2E 34024666463,
Docker 34024666462, Release please 34024666474). Closeout-PR (этот): единая архивная папка, owner-guide,
переписка реестра/памяти, ru-text-правка.

**Ожидает владельца (единственные внешние шаги):**
1. удалить remote-ветку `docs/memory-p3-close` (слитый #39; auto-delete не
   сработал);
3. решение по закрытию #25: цель «Perf ≥0.9 / TBT <200 мс» — перф на пороге
   (0.90), TBT 380 мс не достигнут, метрика шумная; мониторинг продолжает
   ночной cron;
4. проверить, что Render пересобрал прод со свежим main (фича отмены в демо).

**Дальше по желанию:** `/oc fix` и полный цикл на фиче из backlog нет —
кандидаты исчерпаны (#12 закрыт этим PR); новые задачи — из эксплуатации
(следующие ночные замеры, замечания ревью).

**Инвентарь:** PR #1–#41 (открыт только #41, релизный); issues: #6/#10/#12
closed (#12 — после мержа #40), #25 open (мониторинг); релиз v1.1.0 в GitHub Release
(тег `cal-com-v1.1.0`, 2026-09-06), прод ждёт авторедеплоя Render; в main 4 opencode-воркфлоу + 4 перенесённых CI + hexlet-check;
