...
## Точка продолжения (итог 2026-09-07) — читать первым

**Где мы.** Курс закрыт целиком (шаги 1–6, 28/28 требований со следами), перф-цикл
от эксплуатации замкнут: ночной cron второй раз подряд планово success
(34019912835 06.09, 34098493279 07.09 — старт 08:02Z при cron 03:00, задержка
планировщика), дедup #25 комментарием. Находка #25 (блокирующий main thread)
исправлена: PR #46 (маршрутный code splitting, −24% стартового JS) → main
`3928d4c`; прогоны на main зелёные (hexlet-check 34112801686, E2E 34112801633,
Docker 34112801578, Release please 34112801723); #25 закрыт по факту замера
0.98 ≥ 0.9 (FID/TBT вне проваленных аудитов). Релизный цикл в третий раз:
release-PR #47 (1.1.1) открыт release-please — на approve/мерж владельцу.

**Ожидает владельца (единственные внешние шаги):**
1. release-PR #47: approve 4 прогонов `action_required` в UI (E2E, Docker,
   Contract sync, opencode-review — ограничение платформы, прецедент #41),
   затем `/oc review` и мерж → тег `cal-com-v1.1.1`;
2. удалить remote-ветки `docs/memory-p3-close` (слитый #39) и
   `perf/bundle-splitting` (слитый #46; auto-delete не сработал);
3. проверить, что Render пересобрал прод со свежим main (#46) — эффект
   подтвердит следующий плановый замер cron.

**Дальше по желанию:** новых задач из плана нет; мониторинг — ночной cron
(при находках создаст новый issue с меткой perf-report), правки — по его
результатам.

**Инвентарь:** PR #1–#46 слиты, открыт только #47 (релизный); issues #6/#10/#12/#25
closed; релиз v1.1.0 в GitHub Release (тег `cal-com-v1.1.0`), 1.1.1 — за #47;
в main 4 opencode-воркфлоу + 4 перенесённых CI + hexlet-check;
секреты HEXLET_ID/OPENCODE_API_KEY, переменная APP_URL; архив — одна папка
`.agents/archive/` (вне git, включая `docs/`-архив ревью-документов);
скиллы локально: gh-fix-ci, ru-text.
...
- [x] 5.10 Ночной cron: плановый запуск 2026-09-06 состоялся с задержкой
      (best-effort у GitHub: cron 03:00 UTC, факт-старт 07:43:29Z) — run
      34019912835 `schedule` **success** за 1m57s, все шаги зелёные, включая
      гейт чистоты каталога; дедуп подтверждён: issue #25 получил комментарий
      со свежим замером (0.90/TBT 380 мс), новых issue/PR нет
      https://github.com/ivbbest/ai-for-developers-project-387/actions/runs/34019912835
...
- [x] 6.10 Финальный проход по требованиям user-story — таблица 28×«требование
      → доказательство» ниже (раздел «Сверка требований»); все 28 закрыты
      ссылками, последнее — ночной cron 5.10 (run 34019912835)
- [x] 6.11 Финальный V-маршрут reading-guide + закрытие ритуала памяти:
      `docs/reading-guide.md` переписан по факту (статус «шаг 1 не выполнен»
      убран, блоки A–D, V1–V5 с командами и актуальными числами 30/54/78/13/12);
      ритуал — context/MEMORY/learnings/реестр обновлены в этом же PR;
      добавлен `docs/owner-guide.md` (O.6)
...
- Тесты: smoke контракта (30) и стаба (54), vitest бэка (78), contract:check
  (prism-proxy, 13 проверок + Violation-гейт), Playwright e2e (12 сценариев).
...
- [x] O.2 Одна архивная папка вне git: `docs/archive/` (5 файлов, был в git)
      ...
      PR #42 (этот): main после #40 `2e47444`
- [x] O.3 Лишние файлы: дубль `feature-task copy.md` удалён (diff с оригиналом
...
- [x] O.5 Публичная документация через ru-text (README + docs/, архивные —
      нет): грепы типографика/стоп-слов/ИИ-фраз по 12 файлам — чисто, правка одна
      (стоп-слово «является» в файле правил); численные таблицы док приведены к
      фактическим 30/54/78/13/12
- [x] O.6 Док для владельца: суть проекта, как пользоваться и поднимать с
      нуля, на что обращать внимание — `docs/owner-guide.md` (этот PR)
- [x] O.7 Перепроверка всего сделанного: локальный CI-гейт полным прогоном
      (compile/smoke×2/vitest/contract:check/typecheck/build/lint/e2e/docker —
      зелёные, docker-smoke сценария отмены в прод-контейнере) + прогоны CI на
      main после мержей (ссылки в «Точке продолжения») + первый плановый cron
      34019912835; фича #12 взята в работу по указанию владельца → PR #40
- [x] O.8 Реализация фичи «отмена брони» (issue #12, «Опции после курса»):
      `DELETE /api/bookings/{id}` через все слои + спека `booking-cancel.md`;
      PR #40 влит `2e47444` (все проверки зелёные, замечания авторевью
      отработаны, `/oc review` APPROVED); критерии приёмки #12 закрыты
      (чек-лист снят в комментарии треда); релиз 1.1.0 — за #41
