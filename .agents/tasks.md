...
## Точка продолжения (итог 2026-09-07, вечер) — читать первым

**Где мы.** Курс закрыт целиком (шаги 1–6, 28/28 требований со следами), перф-цикл
от эксплуатации замкнут полностью: ночной cron планово success оба утра
(34019912835 06.09, 34098493279 07.09 — старт 08:02Z при cron 03:00, задержка
планировщика), находка #25 исправлена (PR #46, code splitting −24% стартового JS
→ main `3928d4c`), issue #25 закрыт по замеру 0.98 ≥ 0.9. Релизный цикл в третий
раз замкнут владельцем: release-PR #47 — approve прогонов в UI → мерж → main
`a7f780f`, тег и GitHub Release `cal-com-v1.1.1`; все прогоны на main зелёные
(hexlet-check 34114291153, E2E/Docker/Contract sync/Release please — success).
Память по итогам ресёрча — PR #48 → main `afb128d`; эта правка — прямым пушем
в main по явному разрешению владельца 07.09.

**Ожидает владельца:**
1. удалить remote-ветку `docs/memory-close-2026-09-07` (слитый #48; обе ветки из
   прежней очереди — `perf/bundle-splitting`, `docs/memory-p3-close` — уже удалены);
2. следующий плановый cron подтвердит эффект #46 на проде после авторедеплоя
   Render (при находках создаст новый issue с меткой perf-report).

**Дальше по желанию:** открытых PR и issues нет, задач из плана нет; мониторинг —
ночной cron, правки — по его результатам.

**Инвентарь:** PR #1–#48 все слиты, открытых нет; issues #6/#10/#12/#25 closed;
релизы v1.0.3 / v1.1.0 / v1.1.1 в GitHub Release (актуальный тег
`cal-com-v1.1.1`, 2026-09-07); в main 4 opencode-воркфлоу + 4 перенесённых CI +
hexlet-check; секреты HEXLET_ID/OPENCODE_API_KEY, переменная APP_URL; архив — одна
папка `.agents/archive/` (вне git, включая `docs/`-архив ревью-документов);
