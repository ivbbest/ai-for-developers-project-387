...
## Точка продолжения (итог дня 2026-09-05) — читать первым

**Где мы.** Шаги курса 1–4 закрыты полностью (со следами), шаг 5 — всё кроме
ночного cron (5.10), шаг 6 — аудиты 6.1–6.9 закрыты, остаются 6.10/6.11 за
5.10. Релиз v1.0.3 выпущен (тег `cal-com-v1.0.3`). Ревью-цепочка закрыта
целиком: #31 (операционное) + #32 (правки Р4–Р7) + #33 (стратегия, разбор
...
**Проверка нерушимости после ревью-правок (2026-09-05):**
- пин upload-artifact v7.0.1 в e2e.yml (#34): E2E на main зелёный после мержа
  (run 33969038993, `c20f992`);
- гейт `git status --porcelain` + артефакты v7 в schedule (#32/#34): первый
  прогон с гейтом — dispatch 33969893991 на `b0d0c6e` зелёный (13:50 UTC),
  дедup подтверждён (комментарий в #25, новых PR/issue нет);
- авторевью без `issues:read` (#32): зелёный прогон на PR #36 (33969099598);
- issue #25: цель «Perf ≥0.9 / TBT <200 мс» достигнута замером 12:22 UTC
  (0.93 / 180 мс, report.json артефакта run 33965810564), но следующий замер
  (13:50, dispatch 33969893991) — 0.88 / 440 мс: метрика шумная (холодный
  Render/free-tier), устойчивого достижения нет — закрытие по-прежнему
  решение владельца после стабильно зелёных замеров.

**Начало завтра (после 03:00 UTC 2026-09-06):**
1. `gh run list --workflow=opencode-schedule.yml --event schedule --limit 3` —
   был ли ночной прогон, статус (первый cron после свежего мержа иногда
   не срабатывает в ту же ночь — если прогонов нет, зафиксировать, 5.10
   остаётся `[~]` до следующей ночи).
2. Если прогон зелёный: дедуп — в issue #25 свежий комментарий (не новый
   issue); артефакты скачиваются. → 5.10 `[x]` со ссылкой на run.
   Заодно — второй зелёный замер для #25 (критерий достижения цели).
3. 6.10 — финальная сверка требований user-story (чек-лист со ссылками ниже
   по шагам), 6.11 — актуализировать `docs/reading-guide.md` (V-маршрут;
   статус «шаг 1 не выполнен» устарел) + ритуал закрытия памяти.
4. Из #33 в работу: Р11/Р15 = часть 6.11; Р16 — аудит пинов перенесённых CI при
   следующем касании (вне критического пути).

**Что осталось (сжатый список):**
- [~] 5.10 ночной cron (п.1–2 выше)
- [ ] 6.10 сверка требований user-story
- [ ] 6.11 reading-guide + ритуал памяти (включая Р11/Р15 из #33)
- [x] #34 разбор независимого аудита: слит `c20f992`, разбора-таблица в конце
      документа, Р2/Р4 исполнены в том же PR
- [x] #33 мерж + фиксация решений В1–В3: слит `5f7cbc8`; В1 — курс закрыт
      после 5.10+6.10+6.11, В2 — мониторинг агентом/решения владелец, В3 —
      опции после курса (рабочая интерпретация в документе, финальное слово
      за владельцем)
- [x] Пользователь: лимит расходов установлен в кабинете OpenCode Zen
      2026-09-05 (месячный usage limit; сумма — в кабинете, не фиксируется) —
      вторая половина P.3 закрыта, «страховка = фильтр круга вызывающих»
      остаётся запасным слоем
- [ ] Пользователь: удалить слитые remote-ветки-призраки `chore/migrate-app`,
      `docs/scaffold-closure`, `docs/review-archive`, `docs/memory-pr37`
      (удаление remote — только владелец)
- [ ] Опции после курса: `/oc fix` по #12 (критерии есть), закрытие #25 по
      стабильно зелёным замерам (один достигнут: 0.93/180 мс, следующий —
      0.88/440 мс, метрика шумная; решение владельца)

**Процессные отклонения сессии (уроки в learnings):** #28/#29 смержены при
pending авто-ревью (запрошенное `/oc review` было APPROVED; на #31/#32 порядок
«оба ревью до мержа» уже выдержан). Ревью-агенты давали ложные находки
(устаревший base ветки на #21, «PR #32 не существует» на #31 — протухший кэш
прогона): разбирать аргументом со ссылкой, править base, не принимать молча.

**Инвентарь:** PR #1–#38 закрыты/слиты (архив ревью-документов — #37
`e2ae2e1`, память — #38 `3a1f1e6`); открытых PR нет; issues: #6/#10 closed, #12 open (критерии есть, работа не начата),
#25 open (мониторинг; цель достигнута одним замером, метрика шумная — см. выше);
релиз v1.0.3; в main 4 opencode-воркфлоу + 4 перенесённых CI + hexlet-check;
секреты HEXLET_ID/OPENCODE_API_KEY, переменная APP_URL (со слэшем — воркфлоу
нормализует); remote-ветки-призраки (на удаление владельцу): chore/migrate-app,
docs/scaffold-closure, docs/review-archive (слитые).
...
- [x] P.3 Пользователь: секрет `OPENCODE_API_KEY` (значение — только руками;
      ...
      OPENCODE_API_KEY от 2026-09-05T05:58Z); месячный лимит расходов в кабинете
      OpenCode Zen установлен владельцем 2026-09-05 — P.3 закрыт полностью
      (сумма не фиксируется; автопополнение баланса — на совести владельца)
...
## Поручения владельца 2026-09-06 (файл `.agents/archive/feature-task.md`)

- [~] O.2 Одна архивная папка вне git: `docs/archive/` (5 файлов, был в git)
      слит в `.agents/archive/docs/`; в репозитории архивных папок не остаётся;
      мёртвые ссылки на `docs/archive/*` убраны из публичных док.
      Ветка `chore/single-archive` → PR (после #40)
- [~] O.3 Лишние файлы: дубль `feature-task copy.md` удалён (diff с оригиналом
      пуст); `{report,report-digest,perf-report}.{json,md}` — в .gitignore с
      #30; `skills-lock.json` — вне git, очищен до пустого набора. Ожидает
      владельца: удалить remote-ветку `docs/memory-p3-close` (слитый #39)
- [x] O.4 Скиллы: удалены неиспользуемые и не планируемые `grill-me` и
      `supabase-postgres-best-practices` (проект на SQLite; grill-me не
      вызывался ни на одном шаге); остались `gh-fix-ci` (правило сопровождения
      PR) и `ru-text` (O.5). `.opencode/` вне git — изменений в репо нет;
      упоминания `grill-me` вычищены из публичных док
- [~] O.5 Публичная документация через ru-text (README + docs/, архивные —
      нет): типографика, стоп-слова, порядок в таблицах — в том же PR, что O.2
- [~] O.6 Док для владельца: суть проекта, как пользоваться и поднимать с
      нуля, на что обращать внимание — `docs/owner-guide.md` (там же)
- [~] O.7 Перепроверка всего сделанного: локальный CI-гейт полным прогоном
      (все 10 команд green) + свежий docker-smoke + прогоны CI на main после
      мержей; фича #12 из «Опций после курса» взята в работу по указанию
      владельца → PR #40 (contract→e2e, 6 слоёв)
- [~] O.8 Реализация фичи «отмена брони» (issue #12, «Опции после курса»):
      `DELETE /api/bookings/{id}` через все слои + спека `booking-cancel.md`;
      критерии приёмки issue закрыты (см. чек-лист в #12 после мержа);
      PR #40 → релиз 1.1.0 (`feat:`)
