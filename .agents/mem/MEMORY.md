...
- 2026-09-05 | project | Шаг 1 закрыт полностью (кроме деплоя 1.6): план развития влит PR #7 (main `1a888ea`) — баг «длинный email» (воспроизведён: 255→400, 254→201; форма режет молча maxLength=254) + фича «отмена брони» (DELETE нет нигде); гонка двух вкладок исключена (409+перезапрос работают). Эти две записи — будущие issues шага 3 | docs/development-plan.md, PR #7
...
- 2026-09-05 | project | Шаг 3 закрыт: триаж-воркфлоу в main (PR #11), issue А #10 разобран командой (/oc explain + свободный текст → план с критериями приёмки), issue Б #12 — автотриажем (run 33951433854). Issue Б без критериев приёмки — дописать перед `/oc fix` на шаге 4 (задача 4.12). План исправления бага email — в трееде #10 (5 шагов, ветка fix/email-limit) | tasks.md шаг 3
- 2026-09-05 | project | Шаг 4 закрыт, релиз **1.0.3** (тег `cal-com-v1.0.3`, main `5875ed8`): цикл issue→PR→ревью→правки→merge→release пройден (PR #15/#16/#17); авторевью в main пишет от github-actions (write-эскалация D4); App-PR #15 оставил урок: squash-заголовок PR обязан быть Conventional, иначе релиза нет. Issue Б (отмена брони) ещё не чинился — критерии приёмки дописать перед `/oc fix` (перенесено за шаг 5) | tasks.md шаг 4
